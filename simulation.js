...
async function getFiles() {
    ...
    for (i = 1; i <= 5; i++){ householdHistoricData.push( await readCSV(`./data/house${i}.csv`)); }

    return {householdHistoricData};
}

async function createAgents(householdHistoricData, batteryCapacity, batteryBool) {
    console.log('Creating Energy Agents using Ganache');
    let agents = new Array(); let agentNationalGrid = new AgentNationalGrid();

        for (var i=1;i<=10;i++){
            if(i>=8) batteryBool = true;

            let agent = new Agent(batteryCapacity, batteryBool); //constructor of class from prosumer
...
async function init() {
    ...
    console.log('actual accounts',accounts)
    ...
    for (let i= 2184; i < 2185 ; i++) {
        ...
        console.log("Energy Exchanged");
        retData.bidLength = bids.length; retData.askLength = asks.length;

        if (bids.length >= 2  && asks.length  >= 2 ){
            let intersection = calculateIntersection(bids, asks); //price in  wei
    ...
    for (let i= 2184; i < 2531; i++) {   //Calculating Parameters from simulation to plot
